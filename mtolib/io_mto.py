...
def generate_parameters(img, object_ids, sig_ancs, p):
    ...
    if p.verbosity:
        print ("start construct tree again")
...
def make_parser():
    ...
    parser.add_argument('-final_cuttend_name', type=str, help='Name of the object image.')
    parser.add_argument('-shape', type=str, help='The shape of the galaxy.')
    parser.add_argument('-moments_name', type=str, help='The name of the moments csv file.')

    return parser

def generate_moments_based_on_image(params):
    if params.verbosity:
        print ("start calculate moments")
    ...
    with open(params.par_out, mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        
        maxarea = 0
        for row in csv_reader:
            area = (int(row['x_end']) - int(row['x_start'])) * (int(row['y_end']) - int(row['y_start']))
            if area > maxarea:
                finalstartx = int(row['x_start'])
                finalendx = int(row['x_end'])
                finalstarty = int(row['y_start'])
                finalendy = int(row['y_end'])
                maxarea = area


    finalcuttendname = ''
    if params.final_cuttend_name:
        finalcuttendname = params.final_cuttend_name
    else:
        imagenamepaths = params.out.split('/')
        originalname = imagenamepaths[-1]

        originalnamepre = originalname.split('.')[0]
        finaloriname = originalnamepre + '_cuttend' + '.png'
        imagenamepaths[-1] = finaloriname
        finalcuttendname = '/'.join(str(x) for x in imagenamepaths)
