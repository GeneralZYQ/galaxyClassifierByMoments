...
def get_image_parameters(img, object_ids, sig_ancs, params,):
    ...
    headings = ['ID', 'X', 'Y', 'A', 'B', 'theta',  # 'kurtosis',
                           'total_flux', 'mu_max', 'mu_median', 'mu_mean', 'R_fwhm', 'R_e', 'R10', 'R90', 'x_start', 'x_end', 'y_start','y_end','area', 'greylevel']
    ...
    for n in range(len(id_set)):
        ...
        pixel_indices = np.unravel_index(sorted_ids[left_indices[n]:right_indices[n]], img.shape)
        preffixParas = get_object_parameters(img, id_set[n], pixel_indices)
        

        # xIndices = [id_set[n], 'x']
        # yIndices = [id_set[n], 'y']
        # xIndices.extend(pixel_indices[1])
        # yIndices.extend(pixel_indices[0])
...
def get_object_parameters(img, node_id, pixel_indices):
    ...
    # print ("the id is %d and the y and x are  %d and %d", node_id, len(pixel_indices[0]), len(pixel_indices[1]))
    # print ("The min x and y are %d and%d", np.amax(pixel_indices[1]), np.amax(pixel_indices[0]))
